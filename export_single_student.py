...
SECTION_ID = os.getenv("PORTFLOW_SECTION_ID", "72086").strip()
COACH_STUDENTS_JSON = os.getenv("PORTFLOW_COACH_STUDENTS_JSON", "").strip()
SEMESTER_START_DEFAULT = os.getenv("PORTFLOW_SEMESTER_START", "").strip() or "2026-02-12"
SEMESTER_END_DEFAULT = os.getenv("PORTFLOW_SEMESTER_END", "").strip() or "2026-06-30"

GOAL_COLUMNS = [
    ("Overzicht creeren", "OC"),
...
GOAL_CODE_MAP = {name.lower(): code for name, code in GOAL_COLUMNS}
...
COACH_STUDENT_CONFIG = parse_coach_students(COACH_STUDENTS_JSON)
COACH_STUDENT_NAMES = {name for name, _ in COACH_STUDENT_CONFIG}
...
COACH_STUDENT_START_DATES = {
    name: start
    for name, start in COACH_STUDENT_CONFIG
...
def parse_args():
    ...
    parser.add_argument(
        "--students-source",
        choices=["coach", "section", "shared"],
        default="coach",
        help="Bron voor studentenlijst: coach (default), section of shared",
    )
...
def write_skill_summaries(skill_entries, skills_root: Path):
    ...
    for idx, (goal_name, code) in enumerate(GOAL_COLUMNS, start=1):
        ...
        entries = skill_entries.get(goal_name.lower(), [])
        lines = [
            f"Vaardigheid: {goal_name}",
            f"Code: {code}",
...
def write_beroepsproducten_overviews(product_entries, products_root: Path):
    for goal_name in sorted(product_entries.keys(), key=lambda x: x.lower()):
        product_dir = products_root / sanitize_name(goal_name)
        product_dir.mkdir(parents=True, exist_ok=True)
        summary_file = product_dir / "overzicht.txt"
        entries = product_entries[goal_name]

        lines = [
            f"Beroepsproduct: {goal_name}",
            f"Aantal gekoppelde items: {len(entries)}",
            "",
        ]
...
def load_students(token, source, section_id):
    if source == "coach":
        ...
        all_students = extract_students_from_shared(shared)
        if not COACH_STUDENT_NAMES:
            print("Waarschuwing: geen coachstudenten in PORTFLOW_COACH_STUDENTS_JSON; toon volledige shared lijst.")
            return all_students
        ...
        missing_names = sorted(name for name in COACH_STUDENT_NAMES if name not in all_students)
        if missing_names:
            print("Coachstudenten niet gevonden in shared collections:")
...
def main():
    ...
    print(f"Studentbron: {args.students_source}")
    print(f"Aantal studenten: {len(students)}")
    ...
    print("- Beroepsproducten/<doelnaam>")
